package ch.bbv.fsm.impl.transfer.model;

public class TransitionModel {

  private final String guid;

  private final String source;

  private final String target;

  private String effect;

  private String triggerGuid;

  private String guard;

  /**
   * Initialize the object.
   *
   * @param guid The GUID of the model element.
   * @param name The name of the model element.
   * @param source The source from which the transition starts.
   * @param target The target where the transition ends.
   */
  public TransitionModel(final String guid, final String source, final String target) {
    this.guid = guid;
    this.source = source;
    this.target = target;
  }

  public void appendString(final StateMachineModel stateMachineModel, final String indent,
      final StringBuilder str) {
    str.append(indent).append("Transition: ").append(source).append(" -> ").append(target)
        .append('\n');
    if (effect != null) {
      str.append(indent).append('\t').append("Effect: ").append(effect).append('\n');
    }
    if (triggerGuid != null) {
      str.append(indent).append('\t').append("Trigger: ")
          .append(stateMachineModel.triggerFor(triggerGuid).getName()).append('\n');
    }
    if (guard != null) {
      str.append(indent).append('\t').append("Guard: ").append(guard).append('\n');
    }
  }

  public void setEffect(final String value) {
    this.effect = value;
  }

  public void setTriggerGuid(final String guid) {
    this.triggerGuid = guid;
  }

  public String getGuid() {
    return guid;
  }

  public String getSource() {
    return source;
  }

  public String getTarget() {
    return target;
  }

  public String getTriggerGuid() {
    return triggerGuid;
  }

  public String getEffect() {
    return effect;
  }

  public void setGuard(final String value) {
    guard = value;
  }

  public String getGuard() {
    return guard;
  }
}
