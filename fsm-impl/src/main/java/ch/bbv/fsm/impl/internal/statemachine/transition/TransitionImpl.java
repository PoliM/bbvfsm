...
public class TransitionImpl<TStateMachine extends StateMachine<TState, TEvent>, TState extends Enum<?>, TEvent extends Enum<?>>
   ...
   */
  private final List<FsmCall<TStateMachine, TState, TEvent>> actions;
  ...
  /**
   * Recursively traverses the state hierarchy, exiting states along the way, performing the action,
   * and entering states to the target.
   * <hr>
   ...
   * <li>1. The source and target state are the same (self transition) --> perform the transition
   * directly: Exit source state, perform transition actions and enter target state</li>
   * <li>2. The target state is a direct or indirect sub-state of the source state --> perform the
   * transition actions, then traverse the hierarchy from the source state down to the target state,
   * entering each state along the way. No state is exited.
   * <li>3. The source state is a sub-state of the target state --> traverse the hierarchy from the
   * source up to the target, exiting each state along the way. Then perform transition actions.
   * Finally enter the target state.</li>
   * <li>4. The source and target state share the same super-state</li>
   * <li>5. All other scenarios:
   * <ul>
   * <li>a. The source and target states reside at the same level in the hierarchy but do not share
   * the same direct super-state</li>
   ...
   */
  private void fire(final InternalState<TStateMachine, TState, TEvent> source,
      final InternalState<TStateMachine, TState, TEvent> target, final Object[] eventArguments,
  ...
  @Override
  public List<FsmCall<TStateMachine, TState, TEvent>> getActions() {
    return this.actions;
  }
  ...
  private void performActions(final Object[] eventArguments,
      final TransitionContext<TStateMachine, TState, TEvent> context) {
    for (final FsmCall<TStateMachine, TState, TEvent> action : this.getActions()) {
      try {
  ...
  public String toString() {
    return String.format("Transition from state %s to state %s.", this.getSource(),
        this.getTarget());
  }
  ...
  private void unwindSubStates(final InternalState<TStateMachine, TState, TEvent> origin,
    ...
    for (InternalState<TStateMachine, TState, TEvent> o = origin; o != this.getSource(); o =
        o.getSuperState()) {
      o.exit(stateContext);
    }
  }
