...
public class InternalStateImpl<TStateMachine extends StateMachine<TState, TEvent>, TState extends Enum<?>, TEvent extends Enum<?>>
  ...
  /**
   * Collection of transitions that start in this . (Transition<TState,TEvent>.getSource() is equal
   * to this state)
   */
   ...
   * The entry action.
   */
  private FsmCall<TStateMachine, TState, TEvent> entryAction;

  /**
   * The exit action.
   */
  private FsmCall<TStateMachine, TState, TEvent> exitAction;
   ...
   * @param value
   */
  private void checkInitialStateIsASubState(final InternalState<TStateMachine, TState, TEvent> value) {
  ...
  private void checkInitialStateIsNotThisInstance(
    ...
    if (this == newInitialState) {
      throw new IllegalArgumentException(String.format(
          "InternalState {0} cannot be the initial sub-state to itself.", this));
    }
  }
  ...
  public InternalState<TStateMachine, TState, TEvent> enterDeep(
    ...
    final InternalState<TStateMachine, TState, TEvent> lastActiveState =
        stateContext.getLastActiveSubState(this);
  ...
  @Override
  public FsmCall<TStateMachine, TState, TEvent> getEntryAction() {
    return this.entryAction;
  }

  @Override
  public FsmCall<TStateMachine, TState, TEvent> getExitAction() {
  ...
  @Override
  public void setEntryAction(final FsmCall<TStateMachine, TState, TEvent> action) {
    this.entryAction = action;

  }

  @Override
  public void setExitAction(final FsmCall<TStateMachine, TState, TEvent> action) {
  ...
  @Override
  public void setInitialState(final InternalState<TStateMachine, TState, TEvent> initialState) {
  ...
  @Override
  public void setSuperState(final InternalState<TStateMachine, TState, TEvent> superState) {
