...
public class StateMachineTest {
  ...
  public static class RecordEntryActionClass implements FsmAction1<StateMachineTestStateMachine, States, Events, States> {
  ...
  public static class RecordExitActionClass implements FsmAction1<StateMachineTestStateMachine, States, Events, States> {

    @Override
    public void exec(final StateMachineTestStateMachine fsm, final States p1) {
  ...
  /**
   * When a transition between two states at the top level then the exit action of the source state
   * is executed, then the action is performed and the entry action of the target state is executed.
   * Finally, the current state is the target state.
   */
  @Test
  public void executeTransition() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.E);
    testee.start();
  ...
  public void executeTransitionBetweenStatesOnDifferentLevelsDownwards() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.B2);
    testee.start();
  ...
  public void executeTransitionBetweenStatesOnDifferentLevelsUpwards() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.D1b);
    testee.start();
  ...
  /**
   * When a transition between two states with the same super state is executed then the exit action
   * of source state, the transition action and the entry action of the target state are executed.
   */
  @Test
  public void executeTransitionBetweenStatesWithSameSuperState() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.B1);
    testee.start();
  ...
  public void executeTransitionHandledBySuperState() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.C1b);
    testee.start();
  ...
  /**
   * When a transition targets a super-state with {@link HistoryType#DEEP} then the last active
   * sub-state is entered recursively down to the most nested state.
   */
  @Test
  ...
  /**
   * When a transition targets a super-state with {@link HistoryType#NONE} then the initial
  ...
  /**
   * When a transition targets a super-state with {@link HistoryType#SHALLOW} then the last active
   * sub-state is entered and the initial-state of the entered sub-state is entered (no recursive
   * history).
   */
  @Test
  ...
  /**
   * When a transition targets a super-state then the initial-state of this super-state is entered
   * recursively down to the most nested state. No history here!
   */
  @Test
  public void executeTransitionWithInitialSubState() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.A);
    testee.start();
  ...
  /**
   * When the state machine is initializes to a state with sub-states then the hierarchy is
   * recursively traversed to the most nested state along the chain of initial states.
   */
  @Test
  public void initializeStateWithSubStates() {
    final StateMachineTestStateMachine testee =
        stateMachineDefinition.createPassiveStateMachine("executeTransitionBetweenStatesOnDifferentLevelsDownwards", States.D);
    testee.start();
  ...
  /**
   * After initialization the state machine is in the initial state and the initial state is
   * entered. All states up in the hierarchy of the initial state are entered, too.
   */
  @Test
  ...
  public void setUp() {
    ...
    stateMachineDefinition.in(States.A).executeOnEntry(new RecordEntryActionClass(), States.A).executeOnExit(new RecordExitActionClass(), States.A)
        .on(Events.B).goTo(States.B).on(Events.C).goTo(States.C).on(Events.D).goTo(States.D).on(Events.A);

    stateMachineDefinition.in(States.B).executeOnEntry(new RecordEntryActionClass(), States.B).executeOnExit(new RecordExitActionClass(), States.B);

    stateMachineDefinition.in(States.B1).executeOnEntry(new RecordEntryActionClass(), States.B1)
        .executeOnExit(new RecordExitActionClass(), States.B1).on(Events.B2).goTo(States.B2);

    stateMachineDefinition.in(States.B2).executeOnEntry(new RecordEntryActionClass(), States.B2)
        .executeOnExit(new RecordExitActionClass(), States.B2).on(Events.A).goTo(States.A).on(Events.C1b).goTo(States.C1b);

    stateMachineDefinition.in(States.C).executeOnEntry(new RecordEntryActionClass(), States.C).executeOnExit(new RecordExitActionClass(), States.C)
        .on(Events.A).goTo(States.A);

    stateMachineDefinition.in(States.C1).executeOnEntry(new RecordEntryActionClass(), States.C1)
        .executeOnExit(new RecordExitActionClass(), States.C1);

    stateMachineDefinition.in(States.C2).executeOnEntry(new RecordEntryActionClass(), States.C2)
        .executeOnExit(new RecordExitActionClass(), States.C2);

    stateMachineDefinition.in(States.C1a).executeOnEntry(new RecordEntryActionClass(), States.C1a)
        .executeOnExit(new RecordExitActionClass(), States.C1a);

    stateMachineDefinition.in(States.C1b).executeOnEntry(new RecordEntryActionClass(), States.C1b)
        .executeOnExit(new RecordExitActionClass(), States.C1b).on(Events.A).goTo(States.A);

    stateMachineDefinition.in(States.D).executeOnEntry(new RecordEntryActionClass(), States.D).executeOnExit(new RecordExitActionClass(), States.D);

    stateMachineDefinition.in(States.D1).executeOnEntry(new RecordEntryActionClass(), States.D1)
        .executeOnExit(new RecordExitActionClass(), States.D1);

    stateMachineDefinition.in(States.D1a).executeOnEntry(new RecordEntryActionClass(), States.D1a)
        .executeOnExit(new RecordExitActionClass(), States.D1a);

    stateMachineDefinition.in(States.D1b).executeOnEntry(new RecordEntryActionClass(), States.D1b)
        .executeOnExit(new RecordExitActionClass(), States.D1b).on(Events.A).goTo(States.A).on(Events.B1).goTo(States.B1);

    stateMachineDefinition.in(States.E).executeOnEntry(new RecordEntryActionClass(), States.E).executeOnExit(new RecordExitActionClass(), States.E)
        .on(Events.A).goTo(States.A);
