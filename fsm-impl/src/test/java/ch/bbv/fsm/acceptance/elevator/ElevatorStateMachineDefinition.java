...
public class ElevatorStateMachineDefinition extends
  ...
  private void define() {
    ...
    in(State.OnFloor).executeOnEntry(new ElevatorStateMachineDefinition.AnnounceFloorAction()).on(Event.CloseDoor).goTo(State.DoorClosed)
        .on(Event.OpenDoor).goTo(State.DoorOpen).on(Event.GoUp).goTo(State.MovingUp)
